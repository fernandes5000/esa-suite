    ...
    @if ($step == 1)
        <div id="step-1">
            ...
            <div class="flex justify-end mt-8">
                <button type="button" wire:click="saveAndContinue" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">{{ __('Get Started') }}</button>
            </div>
        </div>
    @endif
        ...
        <div id="step-2">
            ...
            <form wire:submit="saveAndContinue">
                <div>
                    ...
                    <input type="text" wire:model="certificate_name" id="certificate_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    @error('certificate_name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="flex justify-between items-center mt-8">
                    <button type="button" wire:click="previousStep" class="text-sm font-medium text-gray-600 hover:text-gray-900">{{ __('Previous Step') }}</button>
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">{{ __('Next Step') }}</button>
                </div>
            </form>
        </div>
    @endif
            ...
            <div class="space-y-4">
                @forelse($userPets as $pet)
                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-md">
                        ...
                        <div class="space-x-4">
                            <button type="button" wire:click="openEditPetModal({{ $pet['id'] }})" class="text-sm font-medium text-blue-600 hover:text-blue-900">{{ __('Edit') }}</button>
                            <button type="button" wire:click="openDeletePetModal({{ $pet['id'] }}, '{{ $pet['name'] }}')" class="text-sm font-medium text-red-600 hover:text-red-900">{{ __('Delete') }}</button>
                        </div>
                    </div>
                @empty
                    <div class="text-center text-gray-500 p-4 border border-dashed rounded-md">{{ __("You haven't added any pets yet.") }}</div>
                @endforelse
            </div>
            <div class="mt-4">
                <button type="button" wire:click="openAddPetModal" class="text-sm font-medium text-blue-600 hover:text-blue-900">+ {{ __('Add a Pet') }}</button>
            </div>
            <div class="flex justify-between items-center mt-8">
                <button type="button" wire:click="previousStep" class="text-sm font-medium text-gray-600 hover:text-gray-900">{{ __('Previous Step') }}</button>
                <button type="button" wire:click="saveAndContinue" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 @if(empty($userPets)) opacity-50 cursor-not-allowed @endif" @if(empty($userPets)) disabled title="{{ __('You must add a pet to continue.') }}" @endif>{{ __('Next Step') }}</button>
            </div>
        </div>
    @endif
            ...
            <form wire:submit="saveAndContinue">
                <div class="space-y-4">
                    @foreach($challengeOptions as $option)
                        <label for="challenge_{{ $option }}" class="flex items-center p-4 border border-gray-200 rounded-md has-[:checked]:bg-blue-50 has-[:checked]:border-blue-300 transition">
        ...
        <div id="step-5">
            ...
            <form wire:submit="saveAndContinue">
                <div>
                    ...
                    <textarea wire:model="description" id="description" rows="6" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"></textarea>
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-4 space-y-4">
                                <div>
                                    ...
                                    <input type="text" wire:model="pet_name" id="pet_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="pet_type" id="pet_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="pet_breed" id="pet_breed" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <input type="number" wire:model="pet_age" id="pet_age" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <textarea wire:model="pet_notes" id="pet_notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"></textarea>
                                    @error('pet_notes') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Save Pet') }}</button>
                            <button type="button" wire:click="closeAddPetModal" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Cancel') }}</button>
                        </div>
                    </form>
                </div>
            </div>
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-4 space-y-4">
                                <div>
                                    ...
                                    <input type="text" wire:model="edit_pet_name" id="edit_pet_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    @error('pet_name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="edit_pet_type" class="block text-sm font-medium text-gray-700">{{ __('Type') }}</label>
                                    <input type="text" wire:model="edit_pet_type" id="edit_pet_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    @error('pet_type') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="edit_pet_breed" class="block text-sm font-medium text-gray-700">{{ __('Breed') }}</label>
                                    <input type="text" wire:model="edit_pet_breed" id="edit_pet_breed" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    @error('pet_breed') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="edit_pet_age" class="block text-sm font-medium text-gray-700">{{ __('Age (optional)') }}</label>
                                    <input type="number" wire:model="edit_pet_age" id="edit_pet_age" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    @error('pet_age') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="edit_pet_notes" class="block text-sm font-medium text-gray-700">{{ __('Notes (optional)') }}</label>
                                    <textarea wire:model="edit_pet_notes" id="edit_pet_notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"></textarea>
                                    @error('pet_notes') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Save Changes') }}</button>
                            <button type="button" wire:click="closeEditPetModal" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Cancel') }}</button>
                        </div>
                    </form>
                </div>
            </div>
